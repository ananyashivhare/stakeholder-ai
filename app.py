...
from auth import get_pending_users, approve_user, deny_user
...
# Session state keeps the user logged in during the session
...
def show_register_page():
    ...
    with st.form('register_form'):
        ...
        submitted = st.form_submit_button('Submit Access Request', type='primary', use_container_width=True)
...
def show_main_app():
    ...
    with st.sidebar:
        ...
        if user['tier'] == 'free':
            st.progress(remaining / FREE_LIMIT, text=f'{remaining}/{FREE_LIMIT} uses remaining today')
            if remaining <= 2:
                st.warning('Running low! Upgrade for unlimited access.')
    ...
    with col1:
        project_name = st.text_input('Project Name', placeholder='e.g. PaymentPro ERP Migration')
    ...
    if not can_use:
        st.error(f'Daily limit reached ({FREE_LIMIT} uses). Resets at midnight or upgrade for unlimited access.')
    ...
    if generate_btn:
        if not update_text.strip():
            ...
            with st.spinner('AI is crafting your personalised messages...'):
                results = personalise_message(update_text, project_name or 'Your Project', selected)
...
def show_admin_panel():
    ...
    for user in pending:
        with st.expander(f"{user['full_name']} - {user['company']}"):
            ...
            st.write(f"Role: {user['job_title']} at {user['company']}")
...
def main():
    ...
    if params.get('admin') == 'true':
        if st.session_state.user and st.session_state.user.get('email') == ADMIN_EMAIL:
            show_admin_panel()
        else:
            ...
            if st.button('Access Admin Panel'):
                if pw == ADMIN_PASSWORD:
                    st.session_state.user = {'email': ADMIN_EMAIL, 'full_name': 'Admin'}
                    st.rerun()
                else:
